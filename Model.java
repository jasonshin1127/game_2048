...
public class Model extends Observable {
    ...
    /** Q1: A new 2048 game on a board of size SIZE with no pieces
     *  and score 0. */
    ...
    public boolean tilt(Side side) {
        ...
        for (int col=0; col<=size()-1;col++){
            for (int i = size()-2; i >= 0; i--){
                Tile t = tile(col,i);
                if (t != null){
                    if (t.value() == tile(col,i+1).value()){
                        _board.move(col,i+1,t);
                        _score += tile(col,i+1).value();
                        changed = true;
                        for (int m = i-1; m >= 0; m--){
                            Tile behind = tile(col, m);
                            if (behind != null){
                                _board.move(col,m+1 ,behind);
                            }
                        }
                    }
                    continue;
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (int i=0;i<b.size();i++){
            for (int k=0;k<b.size()-1;k++){
                if (b.tile(k,i).value() == b.tile(k+1,i).value()){
                    return true;
                }
            }
        }
